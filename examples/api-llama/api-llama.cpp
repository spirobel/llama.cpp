...
void prompt(char *prompt, struct llama_model *model, char *args, void (*prompt_callback)(const char *response, uint64_t len))
{
    ...
    lparams.n_threads_batch = params.n_threads_batch == -1 ? params.n_threads : params.n_threads_batch;
        ...
        for (int32_t i = 0; i < (int32_t)batch.n_tokens; i += n_batch)
        {
            ...
            if (ret != 0)
            {
                if (n_batch == 1 || ret < 0)
                {
                    ...
                    printf("%s : failed to decode the batch, n_batch = %d, ret = %d\n", __func__, n_batch, ret);
                    return;
                }
            ...
            for (auto &client : clients)
            {
                     ...
                     client.response.find('\n') != std::string::npos))
                {
                    ...
                    printf("\033[31mClient %3d, seq %3d/%3d, prompt %4d t, response %4d t, time %5.2f s, speed %5.2f t/s, cache miss %d \033[0m \nInput:    %s\n\033[35mResponse: %s\033[0m\n\n",
